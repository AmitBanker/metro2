...
func (v *validator) isUpperAlphanumeric(s, fieldName, recordName string) error {
	...
	fmt.Printf("%s in %s has uppercase alphanumeric characters\n", fieldName, recordName)
	return nil
}
...
func (v *validator) isAlphanumeric(s, fieldName, recordName string) error {
	...
	fmt.Printf("%s in %s has alphanumeric characters\n", fieldName, recordName)
	return nil
}
...
func (v *validator) isNumeric(s, fieldName, recordName string) error {
	...
	fmt.Printf("%s in %s has numeric characters\n", fieldName, recordName)
	return nil
}
...
func (v *validator) isPhoneNumber(number int64, recordName string) error {
	...
	fmt.Printf("telephone number in %s has valid phone number\n", recordName)
	return nil
}
...
func (v *validator) isValidType(elm field, data, fieldName, recordName string) error {
	...
	if elm.Required == required {
		if elm.Type&numeric > 0 {
			...
			fmt.Printf("%s in %s is a required field present\n", fieldName, recordName)
	...
	if elm.Type&numeric > 0 {
		...
		fmt.Printf("%s in %s has valid value\n", fieldName, recordName)
		return nil
	}
